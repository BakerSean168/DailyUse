...
- [x] 今日任务概览（待办/进行中/已完成数量）
- [x] 目标进度概览（活跃目标进度）
- [x] 今日日程列表
- [x] 即将到期的提醒
- [x] 任务完成趋势图（最近 7 天/30 天）→ 使用 SVG 实现 MiniPieChart
- [x] 目标完成率饼图 → 使用 SVG 实现 MiniPieChart
- [x] 快速操作入口（新建任务/目标/日程）

### 技术验收

- [x] 使用 `@dailyuse/infrastructure-client` 容器获取数据
- [x] 使用纯 SVG 实现图表（MiniPieChart, MiniTrendChart）
- [x] 数据定时刷新（1分钟间隔，可开关）
- [x] 响应式布局（2列网格）
...
├── DashboardView.tsx              # 主视图 (React)
├── index.ts                       # 模块导出
├── components/
│   ├── index.ts                   # 组件导出
│   ├── StatCard.tsx               # 统计卡片
│   ├── TodaySchedule.tsx          # 今日日程
│   ├── UpcomingReminders.tsx      # 即将提醒
│   ├── GoalProgressList.tsx       # 目标进度
│   ├── MiniTrendChart.tsx         # 趋势图 (纯 SVG)
│   ├── MiniPieChart.tsx           # 饼图 (纯 SVG)
