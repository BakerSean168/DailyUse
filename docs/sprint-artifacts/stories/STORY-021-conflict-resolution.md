# STORY-021: 冲突检测与解决

## 📋 Story 概述

**Story ID**: STORY-021  
**Epic**: EPIC-004 (Offline Sync - 多设备数据同步)  
**优先级**: P3  
**预估工时**: 4-5 天  
**状态**: 📋 Ready for Dev  
**前置依赖**: STORY-019 ✅, STORY-020 (网络同步层)

---

## 🎯 用户故事

**作为** 多设备用户  
**我希望** 当不同设备上的修改产生冲突时能够清晰地看到并解决  
**以便于** 我不会丢失任何重要的修改

---

## 🔧 技术背景

### 冲突场景示例

```
设备 A (离线)              设备 B (在线)              服务端
    │                         │                        │
    │  修改标题为「A版本」      │                        │
    │  (base_version=1)       │                        │
    │                         │  修改标题为「B版本」     │
    │                         │ ────────────────────► │
    │                         │                        │  version=2
    │                         │                        │
    │  恢复网络，尝试同步       │                        │
    │ ────────────────────────────────────────────► │
    │                         │                        │  检测到冲突!
    │ ◄────────────────────────────────────────────  │  base_version(1) < version(2)
    │  返回冲突信息            │                        │
    │                         │                        │
    │  显示冲突解决对话框       │                        │
    │                         │                        │
```

### 冲突解决策略

| 实体类型 | 字段 | 策略 | 说明 |
|----------|------|------|------|
| Goal | title | 用户选择 | 标题冲突需要用户决定 |
| Goal | progress | 取较大值 | 进度只增不减 |
| Goal | status | 优先级合并 | completed > active > paused |
| Task | title | 用户选择 | |
| Task | status | 已完成优先 | 任一完成即完成 |
| Task | dueDate | 取较早 | 更紧迫的优先 |
| Reminder | time | 用户选择 | 时间敏感 |
| Setting | * | 后写入优先 | 配置项简单覆盖 |

---

## 📋 验收标准

### AC-1: 冲突检测

- [ ] 正确检测版本号冲突
- [ ] 识别冲突的具体字段
- [ ] 非冲突字段可自动合并

### AC-2: 自动解决

- [ ] progress 字段取较大值
- [ ] status 字段按优先级合并
- [ ] dueDate 字段取较早值
- [ ] 自动解决后不提示用户

### AC-3: 手动解决

- [ ] 冲突时显示解决对话框
- [ ] 支持选择本地/远程版本
- [ ] 支持手动编辑合并
- [ ] 解决后正确同步

### AC-4: 冲突历史

- [ ] 记录所有冲突及解决方案
- [ ] 可查看历史冲突
- [ ] 支持按实体类型筛选

---

## 📝 Tasks/Subtasks

### Task 21.1: 冲突检测服务 [4h]

- [ ] 21.1.1 创建 `ConflictDetectionService` 类
- [ ] 21.1.2 实现版本号比较逻辑
- [ ] 21.1.3 实现字段级差异检测
- [ ] 21.1.4 判断是否可自动合并

**实现参考:**

```typescript
// services/conflict-detection.service.ts
export interface ConflictInfo {
  entityType: EntityType;
  entityId: string;
  localVersion: VersionedEntity;
  remoteVersion: VersionedEntity;
  conflictingFields: string[];
}

export class ConflictDetectionService {
  detectConflict(
    local: VersionedEntity,
    remote: VersionedEntity
  ): ConflictInfo | null {
    // 版本号相同或本地更新，无冲突
    if (local.version >= remote.version) return null;
    
    // 检测字段级冲突
    const conflictingFields = this.findConflictingFields(local, remote);
    
    if (conflictingFields.length === 0) return null;
    
    return {
      entityType: local.entityType,
      entityId: local.id,
      localVersion: local,
      remoteVersion: remote,
      conflictingFields,
    };
  }
}
```

### Task 21.2: 冲突记录服务 [3h]

- [ ] 21.2.1 创建 `ConflictRecordService` 类
- [ ] 21.2.2 实现 `createConflict()` - 保存冲突记录
- [ ] 21.2.3 实现 `getUnresolvedConflicts()` - 获取未解决冲突
- [ ] 21.2.4 实现 `resolveConflict()` - 标记已解决
- [ ] 21.2.5 实现 `getConflictHistory()` - 查询历史

**数据结构:**

```typescript
interface ConflictRecord {
  id: string;
  entityType: EntityType;
  entityId: string;
  localVersion: string;          // JSON
  remoteVersion: string;         // JSON
  resolvedVersion: string | null;
  resolutionStrategy: 'local' | 'remote' | 'merge' | 'manual' | null;
  resolvedAt: number | null;
  resolvedBy: string | null;     // deviceId
  createdAt: number;
}
```

### Task 21.3: 冲突解决器服务 [6h]

- [ ] 21.3.1 创建 `ConflictResolverService` 类
- [ ] 21.3.2 实现 `tryAutoResolve()` - 尝试自动解决
- [ ] 21.3.3 实现 `getFieldAutoStrategy()` - 获取字段策略
- [ ] 21.3.4 实现 `mergeField()` - 字段合并逻辑
- [ ] 21.3.5 实现 `resolveManually()` - 手动解决
- [ ] 21.3.6 解决后触发同步

**合并策略实现:**

```typescript
// 合并策略配置
const strategies: Record<string, Record<string, ResolutionStrategy>> = {
  goal: {
    title: 'manual',
    description: 'manual',
    progress: 'merge',   // Math.max
    status: 'merge',     // 优先级
    updatedAt: 'remote',
  },
  task: {
    title: 'manual',
    status: 'merge',     // completed 优先
    dueDate: 'merge',    // Math.min
    updatedAt: 'remote',
  },
  reminder: {
    time: 'manual',
    message: 'manual',
    enabled: 'merge',    // OR
  },
  setting: {
    '*': 'remote',       // 后写入优先
  },
};
```

### Task 21.4: 冲突历史服务 [2h]

- [ ] 21.4.1 创建 `ConflictHistoryService` 类
- [ ] 21.4.2 实现 `getFormattedHistory()` - 格式化历史
- [ ] 21.4.3 支持按类型筛选
- [ ] 21.4.4 支持分页

### Task 21.5: 冲突解决 UI [4h]

- [ ] 21.5.1 创建 `ConflictResolutionDialog` 组件
- [ ] 21.5.2 显示本地/远程版本对比
- [ ] 21.5.3 高亮冲突字段
- [ ] 21.5.4 支持选择或编辑
- [ ] 21.5.5 确认按钮触发解决

**UI 设计:**

```
┌─────────────────────────────────────────────────────────┐
│  🔄 数据冲突                                       [X]  │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  目标「学习 TypeScript」存在冲突                         │
│                                                         │
│  ┌─────────────────┐    ┌─────────────────┐            │
│  │   本地版本       │    │   云端版本       │            │
│  │                 │    │                 │            │
│  │ 标题: 学习 TS v1│    │ 标题: 学习 TS 最终│            │
│  │ ^^^^^^^^^^^^^^^ │    │ ^^^^^^^^^^^^^^^^^│  ← 冲突   │
│  │ 进度: 70%       │    │ 进度: 60%        │            │
│  │ 状态: active    │    │ 状态: active     │            │
│  └─────────────────┘    └─────────────────┘            │
│                                                         │
│  ○ 使用本地版本                                         │
│  ○ 使用云端版本                                         │
│  ○ 手动合并                                            │
│    ┌─────────────────────────────────────┐             │
│    │ 标题: 学习 TypeScript - 最终版       │             │
│    └─────────────────────────────────────┘             │
│                                                         │
│                    [取消]  [确认解决]                   │
└─────────────────────────────────────────────────────────┘
```

### Task 21.6: 编写测试 [4h]

- [ ] 21.6.1 冲突检测测试
- [ ] 21.6.2 自动合并策略测试
- [ ] 21.6.3 手动解决测试
- [ ] 21.6.4 冲突历史测试

---

## 📐 Dev Notes

### 技术规范

1. **冲突检测时机**
   - Pull 远程变更时检测
   - Push 返回冲突时处理

2. **自动合并条件**
   - 所有冲突字段都有自动策略
   - 策略不为 'manual'

3. **冲突通知方式** (根据用户设置)
   - `immediate`: 立即弹出对话框
   - `taskbar`: 任务栏通知，点击后处理
   - `on-open`: 下次打开应用时提示

### 相关文件位置

```
apps/desktop/src/main/services/
├── conflict-detection.service.ts   # Task 21.1
├── conflict-record.service.ts      # Task 21.2
├── conflict-resolver.service.ts    # Task 21.3
└── conflict-history.service.ts     # Task 21.4

apps/desktop/src/renderer/components/
└── ConflictResolutionDialog.vue    # Task 21.5
```

---

## 🧪 测试场景

### 场景 21.1: 版本冲突检测

```
前置条件：
  - 设备 A 和设备 B 都有同一目标 (version = 1)
  - 设备 A 离线
  
步骤：
  1. 设备 A (离线) 修改目标标题为「标题 A」
  2. 设备 B (在线) 修改目标标题为「标题 B」，同步成功 (version = 2)
  3. 设备 A 恢复网络，尝试同步
  
预期结果：
  - 检测到版本冲突 (本地 baseVersion=1, 服务端 version=2)
  - 生成冲突记录
  - 同步状态变为 conflict
```

### 场景 21.2: 自动合并 - 进度字段

```
前置条件：
  - 同一目标在两台设备上
  
步骤：
  1. 设备 A (离线) 将进度从 50% 改为 70%
  2. 设备 B (在线) 将进度从 50% 改为 60%
  3. 设备 A 恢复网络
  
预期结果：
  - 自动合并，无需用户干预
  - 最终进度 = max(70%, 60%) = 70%
```

### 场景 21.3: 手动解决 - 标题冲突

```
前置条件：
  - 场景 21.1 的冲突状态
  
步骤：
  1. 弹出冲突解决对话框
  2. 用户选择「使用本地版本」
  3. 点击确认
  
预期结果：
  - 目标标题更新为「标题 A」
  - 冲突记录更新：resolutionStrategy = 'local'
  - 生成新的同步事件
```

### 场景 21.4: 冲突历史查询

```
前置条件：
  - 已解决过 3 个冲突
  
步骤：
  1. 打开设置 → 同步历史
  2. 查看冲突列表
  3. 按类型筛选「目标」
  
预期结果：
  - 显示所有已解决的冲突
  - 每条记录显示：实体名称、冲突字段、解决策略、解决时间
  - 筛选后只显示目标类型的冲突
```

---

## 📁 File List

> 实现过程中创建/修改的文件列表

*开发过程中更新*

---

## 📝 Change Log

| 日期 | 变更 | 作者 |
|------|------|------|
| 2025-12-07 | Story 创建 | AI |

---

## 🤖 Dev Agent Record

### Debug Log

*开发过程中的调试记录*

### Completion Notes

*完成时的备注*
