/**
 * @fileoverview Nutstore Sync 适配器单元测试
...
describe('NutstoreSyncAdapter', () => {
  let adapter: NutstoreSyncAdapter;
  ...
  beforeEach(() => {
    adapter = new NutstoreSyncAdapter(credentials);
  });
  ...
  describe('Constructor', () => {
    it('should throw error if username is missing', () => {
      ...
      }).toThrow('Nutstore username (email) is required');
    });
    ...
    it('should throw error if token is missing', () => {
      ...
      }).toThrow('Nutstore password is required');
    });
  });
