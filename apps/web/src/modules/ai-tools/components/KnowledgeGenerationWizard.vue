    ...
    <v-row justify="center">
      <v-col cols="12" :lg="wizardMaxWidth">
        ...
        <div class="wizard-header mb-6">
          ...
          <p class="text-body-1 text-center text-grey-darken-1">
            通过 AI 快速生成系列知识文档，助力您的学习目标
          </p>
        </div>
            ...
            <v-stepper-window>
                ...
                <v-card-text class="step-content">
                  <v-form ref="formRef" @submit.prevent="handleStartGeneration">
                    ...
                    <v-text-field
                      v-model="topic"
                      label="主题"
                      placeholder="例如：健康减肥方法"
                      :rules="[rules.required, rules.topicLength]"
                      :counter="100"
                      variant="outlined"
                      autofocus
                      required
                      class="mb-4"
                      data-test="input-topic"
                    />
                    ...
                    <v-select
                      ...
                      label="文档数量"
                      :items="documentCountOptions"
                      variant="outlined"
                      class="mb-4"
                      data-test="select-document-count"
                    />
                ...
                <v-card-text class="step-content">
                  ...
                  <div class="progress-section mb-6">
                    ...
                    <div class="text-center">
                      ...
                      <p v-if="estimatedTime" class="text-body-2 text-grey">
                  ...
                  <!-- Document Status List -->
                    ...
                    <v-list lines="two" density="comfortable" role="list">
                      <v-list-item
                        v-for="doc in documentPreviews"
                        :key="doc.uuid"
                        data-test="document-status-item"
                        role="listitem"
                      >
                        <template #prepend>
                          <v-icon
                            :icon="getDocStatusIcon(doc.status)"
                ...
                <v-card-text class="step-content">
                  ...
                  <v-row v-if="documents.length > 0">
                    <v-col
                      ...
                      sm="6"
                      lg="4"
                      data-test="doc-preview-col"
                    >
                      <KnowledgeDocumentCard
                        :document="mapToPreview(doc)"
                        @discard="handleDiscard(doc.uuid)"
                        data-test="doc-card"
                      />
                    </v-col>
                  </v-row>
              ...
              <v-stepper-window-item value="4" data-test="step-4" id="step-panel-4" role="tabpanel" :aria-labelledby="'stepper-tab-4'">
                <v-card-text class="step-content text-center">
                  ...
                  <!-- Success Message -->
                  <h2 class="text-h5 font-weight-bold mb-3" id="completion-heading">
                    知识库创建成功！
                  </h2>

                  <p class="text-body-1 text-grey-darken-2 mb-2" id="completion-summary">
                    已生成 <strong>{{ documents.length }}</strong> 篇关于
                    <strong>"{{ topic }}"</strong> 的文档
                  </p>

                  <p class="text-body-2 text-grey mb-6">
                    文档已保存至知识库，您可以随时查看和编辑。
                  </p>

                  <!-- Assertive Live Region announcing completion -->
                  <div v-if="currentStep === 4" aria-live="assertive" class="visually-hidden" data-test="completion-live">
                    知识库创建成功，已生成 {{ documents.length }} 篇关于 “{{ topic }}” 的文档。
                  </div>
                  ...
                  <v-btn
                    ...
                    @click="handleViewInKnowledgeBase"
                    data-test="btn-view-knowledge"
                  >
                    在知识库中查看
                  </v-btn>
                </v-card-text>
        ...
        <v-dialog v-model="showCancelDialog" max-width="400" role="dialog" aria-modal="true" aria-labelledby="cancel-dialog-title" data-test="cancel-dialog">
          <v-card>
            ...
            <v-card-text>
              您确定要取消吗？{{ isGenerating ? '生成进度将丢失。' : '已输入的数据将丢失。' }}
            </v-card-text>
            <v-card-actions>
              <v-spacer />
...
const audienceOptions = [
  { value: 'Beginners', title: '初学者', icon: 'mdi-school' },
  { value: 'Intermediate', title: '进阶者', icon: 'mdi-chart-line' },
  { value: 'Advanced', title: '专家', icon: 'mdi-trophy' },
];

// Validation Rules
const rules = {
  required: (value: string) => !!value || '此字段为必填项',
  topicLength: (value: string) =>
    (value.length >= 1 && value.length <= 100) || '主题长度应在 1-100 个字符之间',
};
...
const isStep1Valid = computed(() => {
  return topic.value.length >= 1 && topic.value.length <= 100;
});
...
/**
 * Start knowledge generation (Step 1 → Step 2)
 */
async function handleStartGeneration(): Promise<void> {
  const isValid = await formRef.value?.validate();
  if (!isValid?.valid) return;

  const request: KnowledgeGenerationRequest = {
    topic: topic.value,
...
function handleCancel(): void {
  ...
  nextTick(() => {
    const dialog = document.querySelector('[data-test="cancel-dialog"]') as HTMLElement | null;
    dialog?.focus();
  });
}
 ...
 */
function handleDiscard(uuid: string): void {
  discardDocument(uuid);
}

/**
 * Navigate to Knowledge Base
 */
function handleViewInKnowledgeBase(): void {
  if (!task.value) return;

  // Extract folder path from task or use default
  const folderPath = encodeURIComponent(`/AI Generated/${task.value.topic}`);

  // Navigate to Knowledge Base with folder query param
  router.push(`/knowledge?folder=${folderPath}`);
}

/**
 * Finish wizard and navigate away
 */
...
/**
 * Map GeneratedDocument to GeneratedDocumentPreview
 */
function mapToPreview(doc: GeneratedDocument): GeneratedDocumentPreview {
  return {
    uuid: doc.uuid,
    title: doc.title,
    status: doc.status,
 ...
 */
function getDocStatusIcon(status: DocumentStatus): string {
  switch (status) {
    case 'GENERATING':
      return 'mdi-sync';
    case 'COMPLETED':
 ...
 */
function getDocStatusColor(status: DocumentStatus): string {
  switch (status) {
    case 'GENERATING':
      return 'blue';
    case 'COMPLETED':
 ...
 */
function getDocStatusText(status: DocumentStatus): string {
  switch (status) {
    case 'GENERATING':
      return '生成中';
    case 'COMPLETED':
...
function focusCurrentStep(): void {
  nextTick(() => {
    ...
    switch (currentStep.value) {
      case 1:
        selector = '[data-test="input-topic"]';
        break;
      case 2:
        selector = '[data-test="progress-bar"]';
        break;
      case 3:
        selector = '[data-test="doc-card"]';
        break;
      case 4:
        selector = '[data-test="btn-view-knowledge"]';
        break;
    }
    if (selector) {
...
// ============================================================
function handleStepperKeydown(e: KeyboardEvent): void {
  const key = e.key;
...
watch(
  ...
  (newRoute) => {
    if (!newRoute.path.includes('knowledge-generator')) {
      reset();
    }
  },
);

// Expose internal methods/state for testing
defineExpose({
