...
/**
 * ONE_TIME Task API 客户端
 * 负责一次性任务相关的API调用
 *
 * API路由:
 * - POST   /tasks/one-time                      - 创建一次性任务
 * - GET    /tasks/one-time                      - 获取任务列表（支持过滤）
 * - GET    /tasks/today                         - 获取今日任务
 * - GET    /tasks/overdue                       - 获取逾期任务
 * - GET    /tasks/upcoming                      - 获取即将到期的任务
 * - GET    /tasks/by-priority                   - 按优先级排序获取任务
 * - GET    /tasks/dashboard                     - 获取任务仪表板
 ...
 * - POST   /tasks/:parentUuid/subtasks          - 创建子任务
 * - GET    /tasks/:parentUuid/subtasks          - 获取子任务列表
 * - POST   /tasks/:uuid/link-goal               - 关联到目标
 * - DELETE /tasks/:uuid/link-goal               - 解除关联
 * - POST   /tasks/batch/update-priority         - 批量更新优先级
 * - POST   /tasks/batch/cancel                  - 批量取消
 */
export class OneTimeTaskApiClient {
  ...
  async createOneTimeTask(
    ...
    const data = await apiClient.post(`${this.baseUrl}/one-time`, request);
    return data;
  }
   ...
   * 开始任务
   */
  async startTask(uuid: string): Promise<TaskContracts.TaskTemplateServerDTO> {
   ...
   * 完成任务
   */
  async completeTask(uuid: string): Promise<TaskContracts.TaskTemplateServerDTO> {
  ...
  async blockTask(
    ...
    const data = await apiClient.post(`${this.baseUrl}/${uuid}/block`, { reason });
    return data;
  }
   ...
   * 解除阻塞
   */
  async unblockTask(uuid: string): Promise<TaskContracts.TaskTemplateServerDTO> {
    const data = await apiClient.post(`${this.baseUrl}/${uuid}/unblock`);
    return data;
  }
  ...
  async cancelTask(
    ...
    const data = await apiClient.post(`${this.baseUrl}/${uuid}/cancel`, { reason });
    return data;
  }
  ...
  async getOneTimeTasks(
    ...
    const data = await apiClient.get(`${this.baseUrl}/one-time`, { params: filters });
    return data;
  }
   ...
   * 获取今日任务
   */
  async getTodayTasks(): Promise<TaskContracts.TaskTemplateServerDTO[]> {
   ...
   * 获取逾期任务
   */
  async getOverdueTasks(): Promise<TaskContracts.TaskTemplateServerDTO[]> {
   ...
   * 获取即将到期的任务
   */
  async getUpcomingTasks(days = 7): Promise<TaskContracts.TaskTemplateServerDTO[]> {
    const data = await apiClient.get(`${this.baseUrl}/upcoming`, { params: { days } });
    return data;
  }
   ...
   * 按优先级排序获取任务
   */
  async getTasksByPriority(limit?: number): Promise<TaskContracts.TaskTemplateServerDTO[]> {
    const data = await apiClient.get(`${this.baseUrl}/by-priority`, { params: { limit } });
    return data;
  }
  ...
  async getTaskDashboard(): Promise<TaskContracts.TaskDashboardResponse> {
    const data = await apiClient.get(`${this.baseUrl}/dashboard`);
    return data;
  }
   ...
   * 获取阻塞的任务
   */
  async getBlockedTasks(): Promise<TaskContracts.TaskTemplateServerDTO[]> {
    const data = await apiClient.get(`${this.baseUrl}/blocked`);
    return data;
  }
  ...
  async getTasksByDateRange(
    ...
    const data = await apiClient.get(`${this.baseUrl}/by-date-range`, {
   ...
   * 按标签获取任务
   */
  async getTasksByTags(tags: string[]): Promise<TaskContracts.TaskTemplateServerDTO[]> {
    const data = await apiClient.get(`${this.baseUrl}/by-tags`, {
   ...
   * 按目标获取任务
   */
  async getTasksByGoal(goalUuid: string): Promise<TaskContracts.TaskTemplateServerDTO[]> {
    const data = await apiClient.get(`${this.baseUrl}/by-goal/${goalUuid}`);
    return data;
  }
   ...
   * 按关键结果获取任务
   */
  async getTasksByKeyResult(keyResultUuid: string): Promise<TaskContracts.TaskTemplateServerDTO[]> {
    const data = await apiClient.get(`${this.baseUrl}/by-key-result/${keyResultUuid}`);
    return data;
  }
  ...
  async createSubtask(
    ...
    const data = await apiClient.post(`${this.baseUrl}/${parentUuid}/subtasks`, request);
    return data;
  }
   ...
   * 获取子任务列表
   */
  async getSubtasks(parentUuid: string): Promise<TaskContracts.TaskTemplateServerDTO[]> {
    const data = await apiClient.get(`${this.baseUrl}/${parentUuid}/subtasks`);
    return data;
  }
  ...
  async linkToGoal(
    uuid: string,
    request: TaskContracts.LinkTaskToGoalRequest,
  ): Promise<TaskContracts.TaskTemplateServerDTO> {
    const data = await apiClient.post(`${this.baseUrl}/${uuid}/link-goal`, request);
    return data;
  }
   ...
   * 解除任务与目标的关联
   */
  async unlinkFromGoal(uuid: string): Promise<TaskContracts.TaskTemplateServerDTO> {
    const data = await apiClient.delete(`${this.baseUrl}/${uuid}/link-goal`);
    return data;
  }
  ...
  async batchUpdatePriority(
    request: TaskContracts.BatchUpdatePriorityRequest,
  ): Promise<TaskContracts.TaskTemplateServerDTO[]> {
    const data = await apiClient.post(`${this.baseUrl}/batch/update-priority`, request);
    return data;
  }
  ...
  async batchCancelTasks(
    request: TaskContracts.BatchCancelTasksRequest,
  ): Promise<TaskContracts.TaskTemplateServerDTO[]> {
    const data = await apiClient.post(`${this.baseUrl}/batch/cancel`, request);
    return data;
  }
  ...
  async updateOneTimeTask(
    ...
    const data = await apiClient.patch(`${this.baseUrl}/${uuid}`, updates);
    return data;
  }
...
// 导出单例实例
export const oneTimeTaskApiClient = new OneTimeTaskApiClient();
