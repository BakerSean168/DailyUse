...
async function handleGenerate() {
    try {
        const result = await generateKnowledgeDocument({ topic: form.value.topic, context: form.value.context || undefined, templateType: form.value.templateType });
        showSuccess('文档生成完成');
        close();
        const content = result.document?.content || '[内容未返回]';
        window.dispatchEvent(new CustomEvent('ai-chat:inject', { detail: { content: `以下是关于 “${form.value.topic}” 的文档草稿:\n\n${content}\n\n请帮助我审阅并提出改进建议。` } }));
    } catch (e: any) { showError(e?.message || '生成文档失败'); }
}
