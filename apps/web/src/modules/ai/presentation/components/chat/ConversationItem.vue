<template>
    <div class="conversation-item" :class="{ active: isActive }"
        @click="$emit('select', conversation.conversationUuid)">
        ...
        <button class="delete-btn" @click.stop="$emit('delete', conversation.conversationUuid)" title="删除"
            aria-label="删除对话"><span>×</span></button>
    </div>
</template>
<script setup lang="ts">
import { computed } from 'vue';
import type { Conversation } from '../../types/conversation';
interface Props { conversation: Conversation; isActive: boolean }
const props = defineProps<Props>();
const displayTitle = computed(() => props.conversation.title || 'New Chat');
...
.conversation-item:hover {
    background: linear-gradient(90deg, rgba(74, 108, 247, .08) 0%, rgba(74, 108, 247, .04) 100%);
}

.conversation-item.active {
    background: linear-gradient(90deg, rgba(74, 108, 247, .15) 0%, rgba(74, 108, 247, .08) 100%);
