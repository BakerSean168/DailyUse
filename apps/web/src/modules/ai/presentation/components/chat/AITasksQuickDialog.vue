    ...
    <v-dialog v-model="show" max-width="640" persistent>
        <v-card>
            <v-card-title class="d-flex align-center">
                <v-icon class="mr-2">mdi-format-list-bulleted</v-icon><span>AI 分解任务</span><v-spacer />
            ...
            <v-card-text>
                <v-alert v-if="quota" :type="hasQuota ? 'info' : 'warning'" variant="tonal" density="compact"
                    class="mb-4">今日剩余：{{ quota.remainingQuota }}/{{ quota.quotaLimit }}</v-alert>
                <v-form ref="formRef" v-model="formValid">
                    ...
                    <v-row>
                        <v-col cols="6"><v-text-field v-model.number="form.targetValue" label="目标值 *" type="number"
                                :rules="[rules.required]" prepend-inner-icon="mdi-flag-checkered"
                                :disabled="isGenerating" /></v-col>
                        <v-col cols="6"><v-text-field v-model.number="form.currentValue" label="当前值 *" type="number"
                                :rules="[rules.required]" prepend-inner-icon="mdi-progress-check"
                                :disabled="isGenerating" /></v-col>
                    </v-row>
                    <v-row>
                        <v-col cols="6"><v-text-field v-model="form.unit" label="单位 (可选)" prepend-inner-icon="mdi-ruler"
                                :disabled="isGenerating" /></v-col>
                        <v-col cols="6"><v-text-field v-model.number="form.timeRemaining" label="剩余天数 *" type="number"
                                :rules="[rules.required]" prepend-inner-icon="mdi-timer"
                                :disabled="isGenerating" /></v-col>
                    </v-row>
                </v-form>
...
async function handleGenerate() { try { const result = await generateTasks({ keyResultTitle: form.value.keyResultTitle, keyResultDescription: form.value.keyResultDescription || undefined, targetValue: form.value.targetValue, currentValue: form.value.currentValue, unit: form.value.unit || undefined, timeRemaining: form.value.timeRemaining }); showSuccess(`生成 ${result.tasks.length} 个任务模板`); close(); const list = result.tasks.map((t: any, i: number) => `${i + 1}. ${t.title || t.name}`).join('\n'); window.dispatchEvent(new CustomEvent('ai-chat:inject', { detail: { content: `已生成任务列表:\n\n${list}\n\n请帮我基于这些任务制定执行优先级与时间安排。` } })); } catch (e: any) { showError(e?.message || '生成任务失败'); } }
