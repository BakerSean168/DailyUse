...
export class AIGenerationController {
  ...
  static async generateKeyResults(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      // 返回成功响应
      res.status(200).json(
        AIGenerationController.responseBuilder.success(result, 'Key results generated successfully'),
      );
    } catch (error) {
  ...
  static async getQuotaStatus(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      res.status(200).json(
        AIGenerationController.responseBuilder.success(quota, 'Quota status retrieved successfully'),
      );
    } catch (error) {
